...
<head>
    ...
    <title>Thêm Sản Phẩm Mới | AI CỦA TÔI Admin</title>
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <main class="content-container">
            ...
            <form action="{{ route('admin.products.store') }}" method="POST" class="form-card" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-input" placeholder="Ví dụ: Tài Khoản Claude Pro" value="{{ old('name') }}" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="slug">Đường dẫn Slug (unique)</label>
                        <input type="text" name="slug" id="slug" class="form-input" placeholder="Ví dụ: claude-pro" value="{{ old('slug') }}" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="category" id="category" class="form-input" required onchange="updateCategoryLabel(this.value)">
                            ...
                            <option value="capcut" {{ old('category') === 'capcut' ? 'selected' : '' }}>Video Editor (CapCut)</option>
                            <option value="gpt" {{ old('category') === 'gpt' ? 'selected' : '' }}>AI Chatbot (ChatGPT)</option>
                            <option value="gemini" {{ old('category') === 'gemini' ? 'selected' : '' }}>Google AI Suite (Gemini)</option>
                            <option value="canva" {{ old('category') === 'canva' ? 'selected' : '' }}>Graphic Design (Canva)</option>
                            <option value="other" {{ old('category') === 'other' ? 'selected' : '' }}>Sản phẩm khác (Claude, Office...)</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="category_label">Nhãn hiển thị danh mục</label>
                        <input type="text" name="category_label" id="category_label" class="form-input" placeholder="Nhãn tự động cập nhật..." value="{{ old('category_label') }}" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group" style="width: 100%;">
                        ...
                        <input type="hidden" name="icon" value="fa-cube">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="default_price" id="default_price" class="form-input" placeholder="Ví dụ: 250000" value="{{ old('default_price') }}" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="default_slashed">Giá gốc gạch ngang (VNĐ)</label>
                        <input type="number" name="default_slashed" id="default_slashed" class="form-input" placeholder="Ví dụ: 600000" value="{{ old('default_slashed') }}" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="rating" id="rating" class="form-input" step="0.1" min="1" max="5" placeholder="4.9" value="{{ old('rating', '4.9') }}" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="review_count">Số lượt đánh giá</label>
                        <input type="number" name="review_count" id="review_count" class="form-input" placeholder="120" value="{{ old('review_count', '120') }}" required>
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label" for="sold">Số lượng đã bán hiển thị</label>
                    <input type="text" name="sold" id="sold" class="form-input" placeholder="Ví dụ: 1.5k" value="{{ old('sold', '1.2k') }}" required>
                </div>

                <h2>Mô tả chi tiết sản phẩm (Hỗ trợ HTML viết tay/rich text)</h2>
                <div class="form-group">
                    <textarea name="description" id="description" class="form-input" rows="8" style="resize: vertical; font-family: inherit;" placeholder="Ví dụ: <p><strong>CapCut Pro chính chủ</strong>...</p>">{{ old('description') }}</textarea>
                </div>

                <h2>Các tính năng nổi bật (Mỗi dòng là một tính năng)</h2>
                <div class="form-group">
                    <textarea name="features" id="features" class="form-input" rows="4" style="resize: vertical; font-family: inherit;" placeholder="Mở khóa toàn bộ hiệu ứng VIP&#10;Nâng cấp trực tiếp chính chủ email&#10;Xuất video chất lượng cao 4K">{{ old('features') }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="seo_title" id="seo_title" class="form-input" placeholder="Ví dụ: Mua tài khoản Claude Pro chính chủ giá rẻ nhất" value="{{ old('seo_title') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="seo_desc">SEO Meta Description (Mô tả tìm kiếm)</label>
                    <textarea name="seo_desc" id="seo_desc" class="form-input" rows="3" style="resize: vertical; font-family: inherit;" placeholder="Nhập mô tả tìm kiếm hấp dẫn giúp tăng lượng click từ Google.">{{ old('seo_desc') }}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label" for="seo_keywords">SEO Keywords (Ngăn cách bởi dấu phẩy)</label>
                    <input type="text" name="seo_keywords" id="seo_keywords" class="form-input" placeholder="Ví dụ: mua tài khoản claude pro, claude pro giá rẻ" value="{{ old('seo_keywords') }}">
                </div>

                <div class="btn-group">
                    <button type="submit" class="btn-submit">Lưu Sản Phẩm <i class="fa-solid fa-save"></i></button>
    ...
    <script>
        ...
        const initialOptions = {!! old('options', $defaultOptionsJson) !!};
